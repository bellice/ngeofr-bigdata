...
files_input = list(INPUT_PATH.rglob("*/*/*/*/TRONCON_DE_ROUTE.shp"))
files_output = [file_output.stem for file_output in OUTPUT_PATH.glob("*-troncons-route-*")]

# Filtre des fichiers qui ne sont pas déjà converti
files_filtered = [
    file
    for file in files_input
    if not any(re.search(r"_D([0-9]{2}[0-9|A-B])\\TRONCON_DE_ROUTE\.shp$", file.__str__()).group(1) in file_output
               and re.search(r"(\d{4}-\d{2})", file.__str__()).group(1) in OUTPUT_PATH
...
for file in files_filtered:
    ...
    code_dep = re.search(r"_D([0-9]{2}[0-9|A-B]).*\\TRONCON_DE_ROUTE\.shp$", file.__str__()).group(1)
    ...
    start_time = time.time()
    df.to_parquet(
        path = OUTPUT_PATH / f"{date}-bdtopo-{code_dep}-troncons-route.parquet",
        compression = "gzip")
