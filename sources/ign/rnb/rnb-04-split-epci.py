# Par rapport à la v2
# Prendre liste large des communes nouvelles depuis 2015 pour éviter les non jointures
# Plus besoin de comparer les deux listes car la commmune sera forcément inscrite
# Il faudrait créer une base plus complète incluant les communes disparues
...
# Chemin dossier
path_input = Path("D:/ign/rnb/parquet/dep")
path_output = Path("D:/ign/rnb/parquet/epci")
version = "2025-02"

base_path = Path.cwd()  # Donne le chemin absolu du répertoire courant
path_db = base_path / "data" / "ngeo2024.parquet"
path_sql = base_path / "shared" / "sql" / "query_epci_ept.sql"

# Vérification de l'existence du fichier Parquet
if not path_db.exists():
    raise FileNotFoundError(f"Le fichier Parquet {path_db} n'existe pas.")
...
# Créer la connexion    
db_ngeofr = duckdb.connect(":memory:")  # Connexion en mémoire
db_ngeofr.execute("INSTALL spatial;")
db_ngeofr.execute("LOAD spatial;")
db_ngeofr.execute(f"CREATE TABLE ngeofr AS SELECT * FROM read_parquet('{path_db}');")
...
# Test epci 200006682 présent sur département 21 et 71
# epci_todo = epci_todo[epci_todo["epci_siren"] == "200006682"]
...
for index, row in epci_todo.iterrows():
    ...
    dep_file_patterns = [f"{version}-pci-{int(dep):03}.parquet" for dep in dep_insee_list]  # Créer les motifs pour les fichiers parquet
    ...
    for dep_pattern in dep_file_patterns:
        matching_files = list(path_input.glob(dep_pattern))  # Rechercher les fichiers correspondant au motif exact
    ...
    if parquet_files:  # S'il y a des fichiers correspondants
        ...
        # Concaténer les fichiers Parquet en une seule vue DuckDB
        parquet_files_str = "', '".join(str(f) for f in parquet_files)  # Liste des fichiers séparés par des virgules

        # Créer la vue pour charger les données de tous les fichiers Parquet
        db.execute(f"""
        ...
        print(f"Aucun fichier Parquet trouvé pour les départements {dep_insee_list}.")
